...
if __name__ == "__main__":
    ...
    algos = {"Жадібний": greedy_algorithm, "Динамічне програмування": dynamic_programming}

    for budget in [25, 50, 75, 100]:
        print(f"\n\nРезультати для бюджету: {budget}\n\n")
        for name, algo in algos.items():
            result = algo(items, budget)
            print(f"Алгоритм {name}:")
            print("Обрані предмети:", result["items"])
            print("Загальна калорійність:", result["calories"])
            print("Загальна вартість:", result["cost"], "\n")
